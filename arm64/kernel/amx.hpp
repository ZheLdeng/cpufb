...
enum matint_alu_mode : uint64_t {
    ...
    /**
     * @brief z + ((x + y) >> s)
     * @note Particular write enable mode can skip x or y
     */
    iadd_shift = 2,
    ...
    /**
     * @brief z >> s or sat(z >> s)
     * @note Shift can be rounding, saturation is optional
     */
    ishift = 4,

    /**
     * @brief sat(z + ((x * y * 2) >> 16))
     * @note Shift is rounding, saturation is signed
     */
    isaturating_fma = 5,
    ...
    /**
     * @brief z + ((x * y) >> s)
     * @note Same as 0, but different lane width modes
     */
    ifma_alt = 8,
    ...
    /**
     * @brief No operation (default for invalid modes)
     */
    ino_op_default = 10
};

enum matint_dtype_t : uint64_t {
  /**
   * @brief xy: i16 z: i32 (ALU mode < 4)
   * xy: 32 z: 16 (ALU mode = 4)
   */
  ...
  /**
   * @brief for default, Do not perform data type conversion
   */
  def = 0
};
enum matint_enable_mode : uint64_t {
  /**
   * @brief config mode by enable value.
   *  0 : Enable all lanes
   *  1 : odd lanes only
...
struct matint : operand<matint> {
   ...
   * @brief only enabled when index_load() == 0
   */
  constexpr auto alu_mode(matint_alu_mode v) const {
    return set_field<47, 6>(v);
  }
  constexpr auto right_shift(uint64_t v) const {
    return set_field<58, 5>(v);
  }

  /**
   * @brief Lane width mode
   */
  constexpr auto dtype_mode(matint_dtype_t v) const { return set_field<42, 4>(v); }
  constexpr auto x_mode(matint_enable_mode v) const {
    return set_field<38, 3>(v);
  }
  constexpr auto x_value(uint64_t v) const { return set_field<32, 5>(v); }
  constexpr auto x_shuffle(shuffle_mode v) const { return set_field<29, 2>(v); }
  constexpr auto y_shuffle(shuffle_mode v) const { return set_field<27, 2>(v); }
  constexpr auto y_mode(matint_enable_mode v) const {
    return set_field<23, 3>(v);
  }
  constexpr auto z_row(uint64_t v) const { return set_field<20, 3>(v); }
  constexpr auto x_offset(uint64_t v) const { return set_field<10, 9>(v); }
